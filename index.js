...
io.sockets.on('connection', function(socket)	{
	...
	socket.emit('message', {message: 'Welcome to Chat Mania', username: "Server", timeString: getDateTimeString()});
	...
	socket.on('setUsername', function(name) {
		socket.set('username', name, function() {
			io.sockets.in(socket.get(socket.room).emit('updateUserlist', io.sockets.clients(socket.get(socket.room)); //updates userList in room
		});
	});
